...
BEGIN
	BEGIN TRY
		...
		INSERT INTO Silver.crm_CustomerInfo (CustomerID, CustomerKey, CustomerLastName, CustomerFirstName, CustomerMaritalStatus, CustomerGender,CustomerCreateDate)	--2.Removing unwanted spaces with a TRIM() function
		SELECT CustomerID, CustomerKey, TRIM(CustomerLastName) AS CustomerLastName, TRIM(CustomerFirstName) AS CustomerFirstName, 
			...
			(CASE UPPER(TRIM(CustomerGender))	--3.Standardizing and normalizing the CustomerGender column using a CASE-WHEN statement	
				...
				WHEN 'F' THEN 'Female'
				ELSE 'Unknown'
			END) AS CustomerGender,CustomerCreateDate
		FROM (
			SELECT CustomerID, CustomerKey, CustomerLastName, CustomerFirstName, CustomerMaritalStatus, CustomerGender,
				CustomerCreateDate, ROW_NUMBER() OVER(PARTITION BY CustomerID ORDER BY CustomerCreateDate DESC) AS LatestDate
